...
function(app, luxon, Viz, vizRenderer, leaflet) {
    ...
    var downloadFirmwareModal = {
        templateUrl: 'app/zigbee2mqtt/downloadFWModal.html',
        controllerAs: '$ctrl',
        controller: function($scope, zigbee2mqtt) {
            var $ctrl = this;
            $ctrl.oldName = $scope.device;
            ...
            $ctrl.downloadFirmware = function() {
                $ctrl.isSaving = true;

                zigbee2mqtt.sendRequest('ota_update', {
                    topic: '/bridge/ota_update/update',
                    state: $ctrl.newName
                }).then(function() {
                    $scope.$close();
                });
    ...
    function Zigbee2MqttDevicesTableController($element, $scope, $uibModal, dzSettings, dataTableDefaultSettings) {
        ...
        $ctrl.$onInit = function() {
            table = $element.find('table').dataTable(Object.assign({}, dataTableDefaultSettings, {
                ...
                columns: [
                    ...
                    { title: 'Manufacturer', width: '60px', data: 'vendor' },
                    { title: 'Model', width: '60px', data: 'model' },
            ...
            table.on('click', '.js-download-fw', function() {
                var row = table.api().row($(this).closest('tr')).data();
                var scope = $scope.$new(true);
                scope.device = row.friendly_name;

                $uibModal.open(Object.assign({ scope: scope }, downloadFirmwareModal));
                $scope.$apply();
            });
        ...
        function actionsRenderer(data, type, row) {
            ...
            actions.push('<button class="btn btn-icon js-download-fw" title="' + $.t('Download firmware') + '"><img src="images/arrow_down.png" /></button>');
            return actions.join('&nbsp;');
        }
    }
});