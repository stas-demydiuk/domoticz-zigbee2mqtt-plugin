...
class Device():
    ...
    def get_device(self, address, alias):
        for unit, device in self.devices.items():
            if device.Options['address'] == address and device.Options['alias'] == alias:
                return device
    ...
    def _create_device(self, message):
        Domoticz.Debug('Creating domoticz device to handle ' + self.value_key + ' key for device with ieeeAddr ' + message.get_device_ieee_addr())
    ...
    def create_device(self, unit, device_name, options, message):
        Domoticz.Error('Unable to create device to handle ' + self.value_key +' value for device ' + message.get_device_ieee_addr())

    def get_numeric_value(self, value, device):
        Domoticz.Error('Device with alias "' + self.alias + '" for key ' + self.value_key + ' can not calculate numeric value')

    def get_string_value(self, value, device):
        Domoticz.Error('Device with alias "' + self.alias + '" for key ' + self.value_key + ' can not calculate string value')
